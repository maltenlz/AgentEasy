...
class EasyNet(nn.Module):
    ...
    def plot_nnet(self):
        ...
        model_graph = draw_graph(
            self, input_size=input_size, graph_name="EasyNet", save_graph=True
        )
...
class DoubleQLearning(ABCQLearning):
    """
    ...
    def initialize_nnets(self, input_dims):
        self.target_net = EasyNet(
            size_scaler=self.learning_config.size_scaler, input_dim=input_dims
        )
    ...
    def learn_from_experience(self, replay_memory):
        ...
        """
        idxs, weights, learning_batch = replay_memory.sample(
            self.learning_config.batch_size
        )
        state, next_state, action, reward, done, legal_moves = (
            self._experiences_to_tensors(learning_batch)
        )
        ...
        future_q_values = (
            self.target_net(next_state).gather(1, next_actions.unsqueeze(1)).squeeze(1)
        )
        ...
        for idx, error in zip(idxs, td_errors, strict=True):
    ...
    def predict(self, state):
        """returns predicted reward for all 19 Tiles 
        (containing illegal moves) based on the current state.
        """
        ...
        with torch.no_grad():
            for target_param, policy_param in zip(
                ...
                target_param.data.copy_(
                    self.learning_config.tau * policy_param.data
                    + (1.0 - self.learning_config.tau) * target_param.data
                )

    @staticmethod
    def _experiences_to_tensors(
