...
class ExplorationConfig:
    ...
    epsilon0: float | None = 0.5
    epsilonmin: float | None = 0.05
    epsilon_decay_rate: float | None = 150000
    tau0: float | None = 0.5
    taumin: float | None = 0.01
    tau_decay_rate: float | None = 0.0001


class ABCExplorationStrategy(ABC):
    """
    ...
    @abstractmethod
    def choose_next_move(self, predictions, possible_moves):
            """Stratgegy to decide on the next move"""

    @abstractmethod
    def plot(self):
        """Plot to see implied schedule given the parameters."""


class EpsilonGreedyStrategy(ABCExplorationStrategy):
    """
    Implementation of Epsilon Greedy Exploration

    Makes a move at random with p = eps_threshold and takes the 
    move with the highest predicted Q-value with p = 1-eps_threshold.
    """
    ...
    def __init__(self, config):
        ...
        assert config.epsilon_decay_rate, (
            "Provide tau_decay_rate for HybridExploration."
        )

    @property
    def eps(self):
        """temperature parameter for epsilon exploration depending on time."""
        epst = self.config.epsilonmin + (
            self.config.epsilon0 - self.config.epsilonmin
        ) * math.exp(-1.0 * self.n_steps / self.config.epsilon_decay_rate)
        if self.n_steps % 1000 == 0:
            print(f"Step {self.n_steps}, temperature: {epst}")
        return epst

    def choose_next_move(self, predictions, possible_moves) -> int:
    ...
    def plot(self):
        ...
        epsilons = [
            self.config.epsilonmin
            + (self.config.epsilon0 - self.config.epsilonmin)
            * math.exp(-1.0 * step / self.config.epsilon_decay_rate)
            for step in steps
        ]
...
class BoltzmannExploration(ABCExplorationStrategy):
    ...
    @property
    def tau(self):
        """temperature parameter for boltzmann exploration depending on time. 1 means 
           spread out and smaller will peak at the best choice"""
    ...
    def plot(self):
        ...
        steps = range(0, 4_000_000, 1_000)
        taus = [self._calc_taut(step) for step in steps]
...
class HybridExploration(ABCExplorationStrategy):
    ...
    def epsilon(self):
        ...
        epst = (
            ...
            * math.exp(-self.config.epsilon_decay_rate * self.n_steps)
            + self.config.epsilonmin
        )
        if self.n_steps % 1000 == 0:
            print(f"Step {self.n_steps}, temperature: {epst}")
